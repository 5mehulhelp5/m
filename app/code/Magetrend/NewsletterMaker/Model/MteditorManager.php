...
class MteditorManager
{
    ...
    public function __construct(
        ...
        \Magetrend\NewsletterMaker\Model\TransportBuilder $transportBuilder,
    ...
    public function sendTestEmail($email, $template, $source)
    {
        ...
        if (version_compare($this->productMetadata->getVersion(), '2.2.8', '<=')) {
            $this->transportBuilder->useLegacyWay(true);
        }
    ...
    public function removeMediaUrl($oHtml)
    {
        $html = explode('/newsletter/', $oHtml);
        if (!isset($html[1])) {
            return $oHtml;
        }
