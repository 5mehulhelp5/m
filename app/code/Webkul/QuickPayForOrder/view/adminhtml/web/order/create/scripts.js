...
define([
...
], function (jQuery, confirm, alert, template, shippingTemplate, paymentTemplate, modal) {
    ...
    AdminOrder.prototype = {
        initialize: function (data) {
            ...
            this.quickpay = false;
            this.quickpayAllowAllShipping = 1;
            this.quickpayAllowAllPayment = 1;
            ...
            jQuery.async('#order-items', (function () {
                ...
                var serviceButtonId = 'add_service_products',
                    addServiceButton = new ControlButton(jQuery.mage.__('Add Service Product'), serviceButtonId);
                ...
                var modelOptions = {
                    ...
                    buttons: [
                        ...
                        },
                        {
                            ...
                            class: 'mymodal2 primary',
                            click: function () {
                                var self = this;
                                var flag = true;
                                var formData = jQuery('#service-model #model-form').serialize();
                                var dataForm = jQuery('#service-model #model-form');
                                if (!dataForm.validation('isValid')) {
                                    return;
                                };
                                var ajaxResponse = order.callServiceModelAjax(formData);
                                var formFields = jQuery('#service-model #model-form #service-tbody-row .service_form_field');
                                jQuery("#service-model #service-message").addClass('hidden');
                                
                                jQuery.each(formFields, function (i, v) {
                                    if (jQuery(v).val() == '') {
                                        flag = false;
                                    }
                                });

                                if (flag && formFields.length > 0) {
                                    ajaxResponse.then(function (response) {
                                        if (response.status == 'ok') {
                                            if (window.serviceId == '') {
                                                window.serviceId = response.productId;
                                            }
                                            order.addProduct(window.serviceId);
                                            jQuery("#model-form-table #service-tbody-row").empty();        
                                            self.closeModal();
                                        } else {
                                            alert({
                                    ...
                                    jQuery("#service-model #service-message").removeClass('hidden');
                                    jQuery("#service-model #service-message").html(jQuery.mage.__('Name and Price fields are required'));
                                }
                            }
                        }
                    ]
                ...
                var popup = modal(modelOptions, jQuery('#service-model'));
                addServiceButton.onClick = function () {
                    jQuery("#service-model").modal("openModal");
                };
                ...
                if (jQuery('#' + this.getAreaId('items')).is(':visible')) {
                    ...
                    this.itemsArea.onLoad = this.itemsArea.onLoad.wrap(function (proceed) {
                        ...
                        if ($(searchAreaId) && !jQuery('#' + searchAreaId).is(':visible') && !$(searchButtonId)) {
                            ...
                            this.addControlButton(addServiceButton);
                        }
                    });
                    this.areasLoaded();
        ...
        callServiceModelAjax: function (formData) {
            var promiseObj = new Promise(function (resolve, reject) {
                jQuery.ajax({
                    url: window.addOptionAjaxUrl,
                    data: {productId: window.serviceId, formData: formData},
                    type: 'POST',
