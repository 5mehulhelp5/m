...
{
    public const ACCOUNT_VALUE = 200;
    ...
    {
        if (method_exists(OrderExtension::class, 'setAmGiftcardOrder')) {
            $extensionAttributes = $this->createPartialMock(OrderExtension::class, ['setAmGiftcardOrder']);
        } else {
            $extensionAttributes = $this->getMockBuilder(OrderExtension::class)
                ->addMethods(['setAmGiftcardOrder',])
                ->getMock();
        }
