...
class ReadHandlerTest extends \PHPUnit\Framework\TestCase
{
    ...
    public function testLoadAttributesWithLoadedExtension()
    {
        ...
        if (method_exists(CreditmemoExtension::class, 'getAmGiftcardCreditmemo')) {
            $extensionAttributes = $this->createPartialMock(
                CreditmemoExtension::class,
                ['getAmGiftcardCreditmemo']
            );
        } else {
            $extensionAttributes = $this->getMockBuilder(CreditmemoExtension::class)
                ->addMethods(['getAmGiftcardCreditmemo'])
                ->getMock();
        }
    ...
    protected function initCreditmemoWithExtension($gCardCreditmemoId = 0)
    {
        if (method_exists(CreditmemoExtension::class, 'getAmGiftcardCreditmemo')) {
            $extensionAttributes = $this->createPartialMock(
                CreditmemoExtension::class,
                ['getAmGiftcardCreditmemo', 'setAmGiftcardCreditmemo']
            );
        } else {
            $extensionAttributes = $this->getMockBuilder(CreditmemoExtension::class)
                ->addMethods(['getAmGiftcardCreditmemo', 'setAmGiftcardCreditmemo'])
                ->getMock();
        }
