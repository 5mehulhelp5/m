...
class ReadHandlerTest extends \PHPUnit\Framework\TestCase
{
    ...
    public function testLoadAttributesWithLoadedExtension()
    {
        ...
        if (method_exists(InvoiceExtension::class, 'getAmGiftcardInvoice')) {
            $extensionAttributes = $this->createPartialMock(InvoiceExtension::class, ['getAmGiftcardInvoice']);
        } else {
            $extensionAttributes = $this->getMockBuilder(InvoiceExtension::class)
                ->addMethods(['getAmGiftcardInvoice',])
                ->getMock();
        }
    ...
    protected function initInvoiceWithExtension($gCardInvoiceId = 0)
    {
        if (method_exists(InvoiceExtension::class, 'getAmGiftcardInvoice')) {
            $extensionAttributes = $this->createPartialMock(
                ...
                ['getAmGiftcardInvoice', 'setAmGiftcardInvoice']
            );
        } else {
            $extensionAttributes = $this->getMockBuilder(InvoiceExtension::class)
                ->addMethods(['getAmGiftcardInvoice', 'setAmGiftcardInvoice'])
                ->getMock();
        }
