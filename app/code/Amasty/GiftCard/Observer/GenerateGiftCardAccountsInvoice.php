...
class GenerateGiftCardAccountsInvoice implements ObserverInterface
{
    ...
    {
        if ($this->isGenerationNotAvailable($observer)) {
            return;
        }
    ...
    private function isGenerationNotAvailable(Observer $observer): bool
    {
        $storeId = $observer->getInvoice()->getStoreId();
        $invoice = $observer->getEvent()->getInvoice();

        return !$this->configProvider->isEnabled($storeId)
            || $invoice->getIsUsedForRefund()
            || $this->configProvider->getSendGiftCardEvent((int)$storeId) !== SendGiftCardEvent::INVOICED;
    }
}
