...
{
    public const LINK_FILED = 'entity_id';
    public const PRODUCT_ID = 10;
    public const ATTRIBUTE_ID = 5;
    ...
    public function testExecute($amounts, $oldAmounts, $deleteCallCount, $saveCallCount)
    {
        ...
        if (method_exists(ProductExtension::class, 'getAmGiftcardPrices')) {
            $extensionAttributes = $this->createMock(ProductExtension::class);
        } else {
            $extensionAttributes = $this->getMockBuilder(ProductExtension::class)
                ->addMethods(['getAmGiftcardPrices',])
                ->getMock();
        }
